...
const CookingDetails = () => {
  ...
  useEffect(() => {
    const fetchRecipe = setTimeout(async () => {
      try {
        const res = await axios.get(`http://localhost:3000/api/cooking/${_id}`);
    ...
    return () => clearTimeout(fetchRecipe);
  }, [_id]);

  const handleDelete = async () => {
    if (confirm("Are you sure you want to delete this recipe?")) {
      try {
        await axios.delete(`http://localhost:3000/api/cooking/${_id}`);
        navigate("/cooking");
      } catch (error) {
        console.error(error);
      }
  ...
  const { name, description, longdescription, chef } = recipe;
